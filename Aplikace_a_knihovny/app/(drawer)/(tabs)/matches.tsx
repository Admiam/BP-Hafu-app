import {ThemedView} from "@/components/ThemedView";
import {ThemedText} from "@/components/ThemedText";
import {Dimensions, FlatList, RefreshControl, StyleSheet} from "react-native";
import ParallaxScrollView from '@/components/ParallaxScrollView';
import {useColorScheme} from "@/hooks/useColorScheme";
import TopBar from "@/components/navigation/TopBar";
import React, {useCallback, useEffect, useState} from "react";
import {
    getCurrentUserFormCollection, getImageByPetID, getLikedPets,
    getPets,
    PetsDocument, PetsDocumentNavigation,
    UserDocument,
    UserPetActionsInput
} from "@/lib/appwrite";
import {DrawerActions, RouteProp, useIsFocused, useNavigation, useRoute} from "@react-navigation/native";
import {Stack, useFocusEffect} from "expo-router";
import ContentLoader from "react-content-loader/native";
import {Rect} from "react-native-svg";
import * as Location from "expo-location";
import {NativeStackNavigationProp} from "@react-navigation/native-stack";
import {FilterCriteria, RootStackParamList, TabStackParamList} from "@/components/navigation/navigation";
import contactCard from "@/components/cards/ContactCard";
import {calculateMonthsFromBirthdate} from "@/utils/dateUtils";


const screenWidth = Dimensions.get("window").width;
const numColumns = 1;
const itemWidth = screenWidth / 2 - 32;
const loadingItemsWidth = screenWidth / 2 - 32;

const MatchesScreen = () => {

    const colorScheme = useColorScheme();
    const [currentUser, setCurrentUser] = useState<UserDocument | null>(null);
    const [pets, setPets] = useState<PetsDocument[]>([]);
    const [loading, setLoading] = useState(true);
    const [actions, setActions] = useState<UserPetActionsInput[]>([]);
    const [petImages, setPetImages] = useState<Record<string, string>>({});
    const [location, setLocation] = useState<Location.LocationObject | null>(null);
    const [refreshing, setRefreshing] = useState(false);

    const TIMER_THRESHOLD_MS = 24 * 60 * 60 * 1000; // 24 hours threshold
    const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
    const route = useRoute<RouteProp<TabStackParamList, 'matches'>>();
    const filter = route.params?.filter as FilterCriteria | undefined;
    const isFocused = useIsFocused();

    useFocusEffect(
        useCallback(() => {
            // nothing special on focus
            return () => {

                // runs when HomeScreen loses focus
                // @ts-ignore
                navigation.setParams({ filter: undefined });

            };
        }, [navigation])
    );

    useEffect(() => {
        if (isFocused) {
            async function fetchData(){
                try {

                    const user = await getCurrentUserFormCollection();
                    setCurrentUser(user);

                    if (!user) return
                    // Fetch both pets and user actions in parallel
                    const [petsData, petActions] = await Promise.all([
                        getPets(),
                        getLikedPets(user.$id),
                    ]);

                    if (!petsData || !petActions) return;

                    // @ts-ignore
                    let likedPets:PetsDocumentNavigation[] = petActions
                        .filter((action: UserPetActionsInput) => action.action === true && action.user.$id === user.$id && action.pet.is_visible)
                        .map((action: UserPetActionsInput) => action.pet);


                    if (filter) {
                        likedPets = likedPets.filter((pet) => {

                            if (filter.selectedBreeds && filter.selectedBreeds.length) {
                                // Turn the pet’s breed field into an array of plain IDs/strings
                                const petBreedIds: string[] = Array.isArray(pet.breed)
                                    ? pet.breed.map(b => (typeof b === "string" ? b : b.$id))
                                    : typeof pet.breed === "string"
                                        ? [pet.breed]
                                        : [pet.breed.$id];

                                // keep the pet if there is ANY overlap
                                const hasMatch = petBreedIds.some(id => filter.selectedBreeds!.includes(id));

                                if (!hasMatch) return false;
                            }


                            if (filter.type && pet.type.type !== filter.type && filter.type !== "all") {
                                return false;
                            }


                            // Apply gender filter
                            if (filter.gender && filter.gender !== "all" && pet.gender.toString() !== filter.gender) {
                                return false;
                            }



                            // Apply age filter
                            if (filter.ageNumber !== undefined && filter.ageNumber !== 0) {
                                const petAgeInMonths = calculateMonthsFromBirthdate(pet.birth_date);
                                const filterAgeInMonths = filter.isMonth ? filter.ageNumber : filter.ageNumber * 12;

                                if (petAgeInMonths > filterAgeInMonths) {
                                    return false;
                                }
                            }

                            // @ts-ignore
                            if (filter.size && filter.size !== "" && pet.size.$id !== filter.size) {
                                return false;
                            }

                            // Apply health filters
                            if (filter.vaccination !== undefined && pet.vaccination !== filter.vaccination) {
                                return false;
                            }

                            if (filter.deworming !== undefined && pet.deworming !== filter.deworming) {
                                return false;
                            }

                            if (filter.chipping !== undefined && pet.chipping !== filter.chipping) {
                                return false;
                            }

                            if (filter.castration !== undefined && pet.castration !== filter.castration) {
                                return false;
                            }

                            if (filter.handicap !== undefined && pet.handicap !== filter.handicap) {
                                return false;
                            }
                            return true;
                        });
                    }

                    if (pets.length > 0) {
                        const imagePromises = pets.map(async (pet) => {
                            try {
                                const imageData = await getImageByPetID(pet.$id);
                                return {petId: pet.$id, imageUrl: imageData[0].fileUrl || ''};
                            } catch (error) {
                                console.error(`Error loading image for pet ${pet.$id}:`, error);
                                return {petId: pet.$id, imageUrl: ''};
                            }
                        });

                        const results = await Promise.all(imagePromises);
                        const imagesMap = results.reduce((acc, {petId, imageUrl}) => {
                            if (typeof imageUrl === "string") {
                                acc[petId] = imageUrl;
                            }
                            return acc;
                        }, {} as Record<string, string>);

                        setPetImages(imagesMap);
                    }

                    setActions(petActions);
                    // @ts-ignore
                    setPets(likedPets);


                } catch (error) {
                    console.error("Error fetching pets:", error);
                } finally {
                    setLoading(false);
                    setRefreshing(false);
                }
            }
            fetchData()
        }
    }, [isFocused]);
    useEffect(() => {
        async function fetchUser() {
            const user = await getCurrentUserFormCollection();
            setCurrentUser(user);
        }
        fetchUser();
    }, []); //fetch current user on mount
    useEffect(() => {
        async function fetchLikedPets() {
            try {
                if (!currentUser || !currentUser.$id) return;

                // Fetch both pets and user actions in parallel
                const [petsData, petActions] = await Promise.all([
                    getPets(),
                    getLikedPets(currentUser.$id),
                ]);

                if (!petsData || !petActions) return;

                // @ts-ignore
                let likedPets:PetsDocumentNavigation[] = petActions
                    .filter((action: UserPetActionsInput) => action.action === true && action.user.$id === currentUser.$id && action.pet.is_visible)
                    .map((action: UserPetActionsInput) => action.pet);


                if (filter) {
                    likedPets = likedPets.filter((pet) => {

                        if (filter.type && pet.type.type !== filter.type && filter.type !== "all") {
                            return false;
                        }

                        // Apply gender filter
                        if (filter.gender && filter.gender !== "all" && pet.gender.toString() !== filter.gender) {
                            return false;
                        }

                        // Apply age filter
                        if (filter.ageNumber !== undefined && filter.ageNumber !== 0) {
                            const petAgeInMonths = calculateMonthsFromBirthdate(pet.birth_date);
                            const filterAgeInMonths = filter.isMonth ? filter.ageNumber : filter.ageNumber * 12;

                            if (petAgeInMonths > filterAgeInMonths) {
                                return false;
                            }
                        }


                        // Apply size filter
                        if (filter.size && filter.size !== "" && pet.size !== filter.size) {
                            return false;
                        }

                        // Apply crossbreed filter
                        if (filter.crossbreed !== undefined && pet.crossbreed !== filter.crossbreed) {
                            return false;
                        }

                        // Apply health filters
                        if (filter.vaccination !== undefined && pet.vaccination !== filter.vaccination) {
                            return false;
                        }

                        if (filter.deworming !== undefined && pet.deworming !== filter.deworming) {
                            return false;
                        }

                        if (filter.chipping !== undefined && pet.chipping !== filter.chipping) {
                            return false;
                        }

                        if (filter.castration !== undefined && pet.castration !== filter.castration) {
                            return false;
                        }

                        if (filter.handicap !== undefined && pet.handicap !== filter.handicap) {
                            return false;
                        }

                        if (!pet.is_visible) {
                            return false;
                        }

                        return true;
                    });
                }

                setActions(petActions);
                // @ts-ignore
                setPets(likedPets);
            } catch (error) {
                console.error("Error fetching liked pets:", error);
            } finally {
                setLoading(false);
            }
        }

        fetchLikedPets();
    }, [currentUser]);
    useEffect(() => {
        async function loadPetImages() {
            if (pets.length > 0) {
                const imagePromises = pets.map(async (pet) => {
                    try {
                        const imageData = await getImageByPetID(pet.$id);
                        return {petId: pet.$id, imageUrl: imageData[0].fileUrl || ''};
                    } catch (error) {
                        console.error(`Error loading image for pet ${pet.$id}:`, error);
                        return {petId: pet.$id, imageUrl: ''};
                    }
                });

                const results = await Promise.all(imagePromises);
                const imagesMap = results.reduce((acc, {petId, imageUrl}) => {
                    if (typeof imageUrl === "string") {
                        acc[petId] = imageUrl;
                    }
                    return acc;
                }, {} as Record<string, string>);

                setPetImages(imagesMap);
            }
        }

        loadPetImages();
    }, [pets]); //load pet images when pets change
    useEffect(() => {
        async function getCurrentLocation() {
            let { status } = await Location.requestForegroundPermissionsAsync();
            if (status !== 'granted') {
                console.error('Permission to access location was denied');
                return;
            }
            let location = await Location.getCurrentPositionAsync({});
            setLocation(location);
        }
        getCurrentLocation();
    }, []); // Get current location on mount

    const onOptions = () => {
        navigation.navigate('Modals', {
            screen: 'Filter',
            params: { pet: pets, navigateTo: "matches" },
        });
    };

    const onMenu = () => {
        navigation.dispatch(DrawerActions.openDrawer());
    }

    const navigateToDetail = (pet: PetsDocument, image: string) => {
        navigation.navigate('Modals', {
            screen: 'PetDetail',
            params: {
                pet: pet,
                image: image
            },
        });
    };

    const navigateToContact = (pet: PetsDocumentNavigation) => {
        navigation.navigate('Modals', {
            screen: 'ContactShelter',
            params: {
                pet: pet,
            },
        });
    };

    const renderLoadingItem = () => (
        <ThemedView style={styles.petItem}>
            <ContentLoader
                height={loadingItemsWidth}
                style={{ width: '100%', borderRadius: 10}}
                speed={1}
                backgroundColor={'#333'}
                foregroundColor={'#999'}
                viewBox={`0 0 ${loadingItemsWidth} ${loadingItemsWidth}`}
            >
                <Rect x="0" y="0" rx="10" ry="10" width={loadingItemsWidth} height={loadingItemsWidth} />
            </ContentLoader>
        </ThemedView>
    );

    const onRefresh = useCallback(async () => {
        setRefreshing(true);
        try {

            const user = await getCurrentUserFormCollection();
            setCurrentUser(user);

            if (!user) return
            // Fetch both pets and user actions in parallel
            const [petsData, petActions] = await Promise.all([
                getPets(),
                getLikedPets(user.$id),
            ]);

            if (!petsData || !petActions) return;

            // @ts-ignore
            let likedPets:PetsDocumentNavigation[] = petActions
                .filter((action: UserPetActionsInput) => action.action === true && action.user.$id === user.$id && action.pet.is_visible)
                .map((action: UserPetActionsInput) => action.pet);


            if (filter) {
                likedPets = likedPets.filter((pet) => {

                    if (filter.selectedBreeds && filter.selectedBreeds.length) {
                        // Turn the pet’s breed field into an array of plain IDs/strings
                        const petBreedIds: string[] = Array.isArray(pet.breed)
                            ? pet.breed.map(b => (typeof b === "string" ? b : b.$id))
                            : typeof pet.breed === "string"
                                ? [pet.breed]
                                : [pet.breed.$id];

                        // keep the pet if there is ANY overlap
                        const hasMatch = petBreedIds.some(id => filter.selectedBreeds!.includes(id));

                        if (!hasMatch) return false;
                    }


                    if (filter.type && pet.type.type !== filter.type && filter.type !== "all") {
                        return false;
                    }


                    // Apply gender filter
                    if (filter.gender && filter.gender !== "all" && pet.gender.toString() !== filter.gender) {
                        return false;
                    }



                    // Apply age filter
                    if (filter.ageNumber !== undefined && filter.ageNumber !== 0) {
                        const petAgeInMonths = calculateMonthsFromBirthdate(pet.birth_date);
                        const filterAgeInMonths = filter.isMonth ? filter.ageNumber : filter.ageNumber * 12;

                        if (petAgeInMonths > filterAgeInMonths) {
                            return false;
                        }
                    }

                    // @ts-ignore
                    if (filter.size && filter.size !== "" && pet.size.$id !== filter.size) {
                        return false;
                    }

                    // Apply health filters
                    if (filter.vaccination !== undefined && pet.vaccination !== filter.vaccination) {
                        return false;
                    }

                    if (filter.deworming !== undefined && pet.deworming !== filter.deworming) {
                        return false;
                    }

                    if (filter.chipping !== undefined && pet.chipping !== filter.chipping) {
                        return false;
                    }

                    if (filter.castration !== undefined && pet.castration !== filter.castration) {
                        return false;
                    }

                    if (filter.handicap !== undefined && pet.handicap !== filter.handicap) {
                        return false;
                    }

                    return true;
                });
            }

            if (pets.length > 0) {
                const imagePromises = pets.map(async (pet) => {
                    try {
                        const imageData = await getImageByPetID(pet.$id);
                        return {petId: pet.$id, imageUrl: imageData[0].fileUrl || ''};
                    } catch (error) {
                        console.error(`Error loading image for pet ${pet.$id}:`, error);
                        return {petId: pet.$id, imageUrl: ''};
                    }
                });

                const results = await Promise.all(imagePromises);
                const imagesMap = results.reduce((acc, {petId, imageUrl}) => {
                    if (typeof imageUrl === "string") {
                        acc[petId] = imageUrl;
                    }
                    return acc;
                }, {} as Record<string, string>);

                setPetImages(imagesMap);
            }

            setActions(petActions);
            // @ts-ignore
            setPets(likedPets);


        } catch (error) {
            console.error("Error fetching pets:", error);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    }, [filter, TIMER_THRESHOLD_MS])

    return (
        <ParallaxScrollView
            headerBackgroundColor={{ light: '#D0D0D0', dark: '#353636' }}
            headerImage={<></>}
        >
            <TopBar
                title="Hafu"
                isShelter={currentUser?.is_shelter}
                leftIcon={{
                    library: "MaterialIcons",
                    name: "menu",
                    onPress: onMenu,
                }}
                rightIcon={{
                    library: "Ionicons",
                    name: "options",
                    onPress: onOptions,
                }}
            />

            <ThemedView className="mt-4" style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                <Stack.Screen options={{ headerShown: false }} />
                <ThemedView className="flex mt-4 mb-16">
                    {loading ? (
                        <FlatList
                            data={Array.from({ length: numColumns * 4 })} // adjust the number of placeholders as needed
                            keyExtractor={(_, index) => index.toString()}
                            renderItem={renderLoadingItem}
                            numColumns={numColumns}
                        />
                    ) : (
                        <FlatList
                            data={pets}
                            keyExtractor={(item) => item.$id}
                            renderItem={({ item }) =>
                                contactCard({
                                    item,
                                    petImages,
                                    location,
                                    navigateToDetail,
                                    //@ts-ignore
                                    navigateToContact,
                                    colorScheme
                                })

                            }
                            refreshControl={
                                <RefreshControl
                                    refreshing={refreshing}
                                    onRefresh={onRefresh}
                                    colors={["#64FCD9", "#689F38"]}
                                    tintColor="#64FCD9"
                                />
                            }
                            numColumns={numColumns}
                            ListEmptyComponent={() => <ThemedText>Nic tu není</ThemedText>}
                        />
                    )}
                </ThemedView>
            </ThemedView>
        </ParallaxScrollView>
    )
}

export default MatchesScreen;

const styles = StyleSheet.create({
    petItem: {
        width: itemWidth,
        margin: 10,
    },
});

