...
import {
    deletePet,
    getImageByPetID,
    getImageByShelerID, getPetDetailByID,
    invisiblePet, PetsDocumentNavigation, UploadedFileInfo, visiblePet
} from "@/lib/appwrite";
...
export default function ShelterPetDetail() {
    ...
    const route = useRoute<RouteProp<ModalStackParamList, 'ShelterPetDetail'>>();
    ...
    useFocusEffect(
        useCallback(() => {
            const fetchPetData = async () => {
                try {
                    const updatedPet = await getPetDetailByID(pet.$id);
                    if (updatedPet)
    ...
    // Function to navigate to shelter detail
    ...
    const handleEdit = (pet: PetsDocumentNavigation, images: UploadedFileInfo[]) => {
        navigation.navigate('Modals', {
            screen: 'UpdatePet',
            params: {
                pet: pet,
    ...
    const fetchPetData = async () => {
        try {
            // Assuming you have a function to get a pet by ID
            const updatedPet = await getPetDetailByID(pet.$id);
            if (updatedPet)
                setPetData(updatedPet);
        } catch (error) {
    ...
    // Function to handle invisible button press
    const handleInvisible = async () => {
        try {

            await invisiblePet(petData.$id);

            Alert.alert("Úspěch", "Váše zvíře bylo zneviditelěno.", [
                {text: "OK",}
            ]);

        } catch (error) {
            console.error("Error invisible pet:", error);
            Alert.alert("Chyba", "Nepodařilo se zneviditelnit zvíře. Zkuste to prosím znovu.");
        }
    };

    // Function to handle visible button press
    const handleVisible = async () => {
        try {

            await visiblePet(petData.$id);

            Alert.alert("Úspěch", "Váše zvíře bylo zviditelěno.", [
                {text: "OK",}
            ]);

        } catch (error) {
            console.error("Error visible pet:", error);
            Alert.alert("Chyba", "Nepodařilo se zviditelnit zvíře. Zkuste to prosím znovu.");
        }
    };
    ...
    const handleDelete = async () => {
        ...
        Alert.alert(
            "Potvrzení smazání",
            "Opravdu chcete smazat tohoto mazlíčka? Tato akce je nevratná.",
            [
                {
                    text: "Ne, zrušit",
                    style: "cancel"
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            Alert.alert("Úspěch", "Váše zvíře bylo odstraněno.", [
                                {text: "OK"}
                            ]);
                            ...
                            Alert.alert("Chyba", "Nepodařilo se odstranit zvíře. Zkuste to prosím znovu.");
                        }
                    }
                }
            ]
        ...
        <ThemedView className="flex-1">
            ...
            <ScrollView contentContainerStyle={{flexGrow:1}}>
                ...
                <ThemedView className="pb-16 flex flex-column space-y-4">
                        ...
                        <ThemedView className="flex flex-col divide-y-2 divide-y-reverse divide-neutral-700 py-3">
                            ...
                            {pet.crossbreed === true && (
                                <ThemedView className="flex-row py-3">
                                    ...
                                    <ThemedText className="ml-2">Kříženec</ThemedText>
                                </ThemedView>
                            )}
                            <ThemedView className="flex-row py-3">
                    ...
                    <EditButton handlePress={() => handleEdit(pet, petImages)} />
                    {pet.is_visible ? (
                        <InvisibleButton buttonText="DOČASNĚ ZNEVIDITELNIT" isVisible={pet.is_visible} handlePress={handleInvisible} />
                    ):(
                        <InvisibleButton buttonText="ZVIDITELNIT" isVisible={pet.is_visible} handlePress={handleVisible} />
                    )}
                    <DeleteButton handlePress={handleDelete} />
                </ThemedView>
